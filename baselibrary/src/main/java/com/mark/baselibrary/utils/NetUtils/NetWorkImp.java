...
import com.mark.baselibrary.R;
import com.mark.baselibrary.base.BaseApplication;
...
public abstract class NetWorkImp<T> implements OnResponseListener<T> {
    ...
    public void onFailed(int what, Response<T> response) {
        ...
        if (exception instanceof NetworkError) {// 网络不好
            ToastHelp.show(BaseApplication.getInstance().getApplicationContext(), R.string.error_please_check_network);
        } else if (exception instanceof TimeoutError) {// 请求超时
            ToastHelp.show(BaseApplication.getInstance().getApplicationContext(), R.string.error_timeout);
        } else if (exception instanceof UnKnownHostError) {// 找不到服务器
            ToastHelp.show(BaseApplication.getInstance().getApplicationContext(), R.string.error_not_found_server);
        } else if (exception instanceof URLError) {// URL是错的
            ToastHelp.show(BaseApplication.getInstance().getApplicationContext(), R.string.error_url_error);
